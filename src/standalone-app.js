...
app.innerHTML = `
  <div class="shell">
    <header class="hero">
      <div><h1>视频 → MP3</h1><p>单文件离线版：音视频分离与 MP3 转换全部在本机浏览器完成。</p></div>
      <div class="badge">🔒 完全离线 · 文件不上传</div>
    </header>
    <section class="card">
      <div id="drop" class="drop">
        <h2>把视频拖到这里</h2>
        <p>FFmpeg 引擎已经内置在这个 HTML 文件中，不需要网络、不需要安装软件。手机可直接从“照片”或“文件”选择。</p>
      ...
      <div id="panel" class="file-panel hidden">
        ...
        <div class="grid">
          ...
          <div class="control"><label>离线处理模式</label><div style="line-height:1.55">优先用 WORKERFS 直接读取本地视频，减少手机内存复制；音轨识别使用 FFmpeg 媒体日志，不调用可能导致部分移动浏览器异常的 ffprobe。</div></div>
        </div>
    ...
    <div class="footer">FFmpeg WebAssembly 引擎已完整内置在本 HTML 中。可以直接复制、离线保存和分发；无需 CDN、Service Worker、服务器或额外文件。<br />超大视频仍会受手机浏览器内存和系统资源限制。</div>
  </div>`;
...
function recentLogs(max = 18) { return logBuffer.slice(-max).join('\n').trim(); }
...
function showSizeWarning(file) {
  ...
  if (isMobile && mb >= 800) showWarning(`这段视频约 ${formatBytes(file.size)}。单文件版会优先直接挂载文件，但手机浏览器仍可能因系统资源限制中断。建议保持页面前台并避免锁屏。`);
  else if (!isMobile && mb >= 4096) showWarning(`这段视频约 ${formatBytes(file.size)}。超大文件在 WebAssembly 中仍可能受到浏览器内存和文件系统限制。`);
}
...
async function ensureFFmpeg() {
  ...
  try {
    await ffmpeg.load((ratio, decoded, total) => setProgress(ratio, `正在展开内置 FFmpeg 引擎：${formatBytes(decoded)} / ${formatBytes(total)}`));
    ffmpegLoaded = true;
    setProgress(1, '内置转换引擎已就绪', '100%');
  } catch (error) {
    ffmpeg = null;
    ffmpegLoaded = false;
    setProgress(0, '内置转换引擎初始化失败');
    throw error;
  }
}
...
async function cleanupInput() {
  ...
  } else if (inputMode === 'memfs' && inputPath) await ffmpeg.deleteFile(inputPath).catch(() => {});
  inputPath=''; inputMode='';
}
async function switchInputToMemory(file, reason) {
  const limit = isMobile ? MOBILE_MEMORY_FALLBACK_LIMIT : DESKTOP_MEMORY_FALLBACK_LIMIT;
  if (file.size > limit) throw new Error(`${reason}，且文件 ${formatBytes(file.size)} 过大，不适合整文件复制进浏览器内存。请换用支持直接挂载的浏览器，或在电脑端处理。`);
  await cleanupInput();
...
async function attachInputFile(file) {
  ...
  try {
    ...
    const mounted = await ffmpeg.mount('WORKERFS', {files:[file]}, MOUNT_POINT);
    if (!mounted) throw new Error('WORKERFS 不可用');
    inputPath = `${MOUNT_POINT}/${file.name}`;
    inputMode = 'workerfs';
    setProgress(1, '视频已直接挂载，准备分析…');
  } catch (error) {
    console.warn('WORKERFS 挂载失败，切换内存模式', error);
    await switchInputToMemory(file, '浏览器无法直接挂载该视频');
  }
}
...
function parseTracksFromLogs(lines) {
  ...
  for (const line of lines) {
    ...
    const parts=audio.split(',').map((p)=>p.trim());
    const codec=parts[0] || '未知编码';
    const samplePos=parts.findIndex((p)=>/\d+\s*Hz/i.test(p));
    ...
    tracks.push({ordinal:tracks.length,streamIndex,lang,codec,sampleRate:sampleRate?`${sampleRate} Hz`:'',channelLayout,bitrate:bitrate?`${Math.round(Number(bitrate))} kb/s`:''});
  }
  return tracks;
}
async function analyzeInput() {
  ...
  let code=-1;
  try { code=await ffmpeg.exec(['-hide_banner','-i',inputPath,'-map','0:a?','-c','copy','-frames:a','1','-f','null','-']); }
  catch (error) { console.warn('FFmpeg 探测命令返回异常，继续解析已输出的媒体信息', error); }
  ...
  const formatName=inputLine.match(/Input #0,\s*(.+?),\s*from\s/i)?.[1] || 'FFmpeg';
  if (!inputLine && !tracks.length) throw new Error(`FFmpeg 无法读取这个视频的媒体信息（返回码 ${code}）。${recentLogs(24)?`\n${recentLogs(24)}`:''}`);
  return {duration,formatName,tracks};
}
function renderTracks(tracks) {
  if (!tracks.length) { tracksEl.innerHTML='<div class="warning">已经读取到媒体文件，但没有发现音频流。这个视频可能确实不含音频，或当前 FFmpeg WebAssembly 构建不支持该音轨。</div>'; convertBtn.disabled=true; return; }
  tracksEl.innerHTML=tracks.map((t,i)=>{
    const detail=[t.codec,t.channelLayout,t.sampleRate,t.bitrate,`Stream #0:${t.streamIndex}`].filter(Boolean).map(escapeHtml).join(' · ');
    return `<label class="track"><input type="checkbox" name="track" value="${t.streamIndex}" ${i===0?'checked':''} /><div><strong>音轨 ${i+1} · ${escapeHtml(t.lang)}</strong><span>${detail}</span></div></label>`;
  }).join('');
  convertBtn.disabled=false;
}
function classifyAnalysisError(error,file) {
  const message=error?.message || String(error), lower=message.toLowerCase();
  ...
  if (/worker|wasm|engine|initialize|initializ/.test(lower)) return `分析失败：内置 FFmpeg 引擎初始化或运行异常。\n${message}`;
  return `分析失败：FFmpeg 无法解析该视频容器或媒体结构。\n${message}`;
}

async function loadFile(file) {
  if (!file || busy) return;
  currentFile=file; currentTracks=[]; panel.classList.remove('hidden'); fileName.textContent=file.name; fileMeta.textContent=`${formatBytes(file.size)} · 正在准备分析…`; showSizeWarning(file); tracksEl.innerHTML=''; resetOutput(); convertBtn.disabled=true;
  try {
    busy=true; statusBox.classList.remove('hidden'); cancelBtn.classList.remove('hidden');
    await ensureFFmpeg(); await attachInputFile(file); const media=await analyzeInput(); currentTracks=media.tracks;
    fileMeta.textContent=`${formatBytes(file.size)} · ${formatDuration(media.duration)} · ${media.formatName} · ${media.tracks.length} 条音轨 · ${inputMode==='workerfs'?'直接挂载':'内存兼容模式'}`;
    renderTracks(media.tracks); setProgress(1,'分析完成');
  } catch (error) {
    console.error(error); statusText.textContent='分析失败'; logEl.textContent=error?.message||String(error); showWarning(classifyAnalysisError(error,file)); await cleanupInput().catch(()=>{});
...
async function convertSelected() {
  ...
  busy=true; resetOutput(); statusBox.classList.remove('hidden'); cancelBtn.classList.remove('hidden'); convertBtn.disabled=true;
  try {
    await ensureFFmpeg(); const stem=safeStem(currentFile.name);
    for (let i=0;i<selected.length;i++) {
      const track=selected[i], outputName=`${stem}_音轨${track.ordinal+1}.mp3`;
      setProgress(0,`正在转换音轨 ${track.ordinal+1}（${i+1}/${selected.length}）…`); logBuffer=[];
      const code=await ffmpeg.exec(['-i',inputPath,'-map',`0:${track.streamIndex}`,'-vn','-c:a','libmp3lame',...qualityArgs(),'-map_metadata','-1','-y',outputName]);
      if (code!==0) throw new Error(`音轨 ${track.ordinal+1} 转换失败（FFmpeg 返回码 ${code}）${recentLogs()?`\n${recentLogs()}`:''}`);
      const data=await ffmpeg.readFile(outputName); const blob=new Blob([data],{type:'audio/mpeg'}); const url=URL.createObjectURL(blob); resultURLs.push(url);
      const item=document.createElement('div'); item.className='result'; item.innerHTML=`<div><strong>${escapeHtml(outputName)}</strong><span>${formatBytes(blob.size)} · MP3 · Stream #0:${track.streamIndex}</span></div><a class="secondary" href="${url}" download="${escapeHtml(outputName)}" style="text-decoration:none;text-align:center">保存 MP3</a>`; resultsEl.appendChild(item); await ffmpeg.deleteFile(outputName).catch(()=>{});
    }
    setProgress(1,`完成：已生成 ${selected.length} 个 MP3`);
  } catch (error) { console.error(error); statusText.textContent='转换失败'; logEl.textContent=error?.message||String(error); showWarning(`转换失败：${error?.message||String(error)}`); }
  finally { busy=false; cancelBtn.classList.add('hidden'); convertBtn.disabled=false; }
}
async function cancelWork() {
  if (!busy || !ffmpeg) return;
  try { ffmpeg.terminate(); } catch {}
  ffmpeg=null; ffmpegLoaded=false; inputPath=''; inputMode=''; currentTracks=[]; busy=false; cancelBtn.classList.add('hidden'); convertBtn.disabled=true; setProgress(0,'已取消。再次选择视频时会重新展开内置引擎。');
}
...
drop.addEventListener('drop',(event)=>{event.preventDefault();drop.classList.remove('drag');const file=event.dataTransfer?.files?.[0];if(file)loadFile(file);});
